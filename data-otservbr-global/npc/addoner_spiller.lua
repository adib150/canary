...
local talkState = {}
    ...
    if addoninfo[message] ~= nil then
        ...
        local items_list = ''
        local storageID = addoninfo[message].storageID
        local sex = player and player:getSex() or 0
        local currentLook = (sex == 0) and addoninfo[message].outfit_female or addoninfo[message].outfit_male
        local addonNum = addoninfo[message].addon
        if currentLook and canPlayerWearOutfit(creature, currentLook, addonNum) then
            npcHandler:say('You already have this addon! I will not take any items.', npc, creature)
            npcHandler:resetNpc(creature)
            return true
        end

        if (storageID and getPlayerStorageValue(creature, storageID) ~= -1) then
    ...
    elseif npcHandler:getTopic(playerId) == 2 then
        if MsgContains(message, "yes") then
            ...
            if(getPlayerMoney(creature) >= addoninfo[rtnt].cost) and (items_number == table.maxn(addoninfo[rtnt].items)) then
                local maleOutfit = addoninfo[rtnt].outfit_male
                local femaleOutfit = addoninfo[rtnt].outfit_female
                local addonNumber = addoninfo[rtnt].addon

                -- Try to add the outfit addons first. If that fails, don't consume items.
                local addedMale = true
                local addedFemale = true
                if maleOutfit then
                    addedMale = doPlayerAddOutfit(creature, maleOutfit, addonNumber)
                end
                if femaleOutfit then
                    addedFemale = doPlayerAddOutfit(creature, femaleOutfit, addonNumber)
                end

                if not addedMale and not addedFemale then
                    npcHandler:say('Sorry, I could not add the addon to your outfit. Please contact an admin.', npc, creature)
                    rtnt = nil
                ...
                -- Now remove money and required items (we already checked counts above)
                ...
                local storageIDYes = addoninfo[rtnt] and addoninfo[rtnt].storageID
                if storageIDYes then
                    setPlayerStorageValue(creature, storageIDYes, 1)
                else
                    npcHandler:say('Configuration warning: addon storage key missing; addon was granted but not tracked.', npc, creature)
                end
